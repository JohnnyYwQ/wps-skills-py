...
> 此外，`call.py` 只是 `/dispatch` HTTP 端点的薄封装——需要程序化批量构建时，**直接 `POST http://127.0.0.1:58891/dispatch` 完全等价且更可控**（无需为完成任务额外编写驱动脚本）：
> ```bash
> curl -X POST http://127.0.0.1:58891/dispatch -H "Content-Type: application/json" \
>   -d '{"action":"addSlide","params":{"layout":"blank","title":"封面"}}'
> ```
...
- 默认 `WPS_TRACE=info` 只记录时间、Action、路由应用、控制器/ProgID、平台后端、PowerShell PID、`reqId`、重试次数、耗时和错误，不记录完整参数或文档内容。
- 实机复现前设置 `WPS_TRACE=debug` 可增加**脱敏后的**参数/响应摘要：凭据字段会变成 `<redacted>`，正文只记录长度与哈希，便于判断两次输入是否相同而不落原文。该环境变量应在首次启动桥接服务前设置。
...
call.py（胶水层：自动拉起服务 + POST）
   │  HTTP JSON  →  127.0.0.1:58891/dispatch
   ▼
...
>
> **重名 Action 不允许猜测应用**：归属唯一的 Action 仍自动路由；`findReplace`（Excel/Word）与 `insertImage`（PPT/Word）必须用 CLI `--app` 或 HTTP 顶层 `app` 明确应用。未指定时返回 `AMBIGUOUS_ACTION`，指定了不支持该 Action 的应用时返回 `ACTION_NOT_SUPPORTED_FOR_APP`。

直接 HTTP 调用重名 Action 时，`app` 与 `action` 同级，路由字段不会传给应用控制器：
...
# 4. Action trace / 路由 / 控制器重试单元测试（不需要 Windows/WPS）
PYTHONPATH=bridge python -m unittest bridge/test_action_trace.py bridge/test_server_routing.py bridge/test_controller_trace.py
```
...
| 6 | PowerShell 5.1 下 `call.py "<json>"` 因双引号 `json.loads` 必挂（`--`/`--%`/`cmd /c` 均失败） | 宿主 shell 对 JSON 引号转义不兼容 | `call.py` 新增 `--params-file <path>` 与 `--stdin`，把 JSON 落盘/走管道，彻底绕开命令行引号；文档同时给出直接 `POST /dispatch` 的等价调用 |
| 7 | 含中文的多行 PowerShell 长命令 `MissingExpressionAfterToken` 解析崩溃 | PowerShell 多行中文解析缺陷 | 桥接命令本就是单行 JSON/逐 action；模型层改用 `--params-file`/直接 HTTP 后不再需要嵌中文多行 PS |
...
| 10 | 被迫绕过 `call.py` 直接调 HTTP 端点（未审计脚本） | `call.py` CLI 形式在本机不通用 | `call.py` 已通用化（`--params-file`/`--stdin`）；并明确"直接 `POST /dispatch` 完全等价"，无需为任务另写驱动脚本 |
