...
def main():
    ...
    r = api_post("/dispatch", {"action": "ping", "params": {}})
    print(f"    结果: {json.dumps(r, ensure_ascii=False)}")
    print()

    print("[4] Excel 连接 GET AppInfo {action: getAppInfo, app: excel}")
    r = api_post("/dispatch", {"action": "getAppInfo", "params": {"app": "excel"}})
    print(f"    结果: {json.dumps(r, ensure_ascii=False)}")
    print()

    print("[5] PPT 连接 GET AppInfo {action: getAppInfo, app: ppt}")
    r = api_post("/dispatch", {"action": "getAppInfo", "params": {"app": "ppt"}})
    print(f"    结果: {json.dumps(r, ensure_ascii=False)}")
    print()

    print("[6] Word 连接 GET AppInfo {action: getAppInfo, app: word}")
    r = api_post("/dispatch", {"action": "getAppInfo", "params": {"app": "word"}})
    print(f"    结果: {json.dumps(r, ensure_ascii=False)}")
    print()
