...
TOP_LEVEL_FIELDS = {"schema_version", "actions", "routing_defaults"}
CONTRACT_FIELDS = {
    "owner",
    "action",
    "description",
...
def load_action_manifest(path=MANIFEST_PATH):
    ...
    routing_defaults = manifest.get("routing_defaults", {})
    if not isinstance(routing_defaults, dict):
        _manifest_error("routing_defaults", "must be object")
    for action, owner in routing_defaults.items():
...
class ActionCatalog:
    ...
    def __init__(self, manifest):
        ...
        self._routing_defaults = dict(manifest.get("routing_defaults", {}))
    ...
    def routing_default(self, action):
        return self._routing_defaults.get(action)
